"""
Содержит функции и хэндлеры команды high_price.
...
from states.information_command_highprice import SearchToHighPrice
...
def high_price(message: Message) -> None:
    ...
    bot.set_state(message.from_user.id, SearchToHighPrice.city, message.chat.id)
    logger.info(f'{message.from_user.full_name} выбрал команду high_price')
...
@bot.message_handler(state=SearchToHighPrice.city)
@logger.catch()
    ...
    * reply_markup - Выводит клавиатуру для выбора района.
    ...
    if city_group[0]["name"] == city:
        bot.send_message(message.from_user.id, 'Уточните, пожалуйста.',
                         reply_markup=choose_place(city_group, 'h'))
...
@bot.callback_query_handler(func=lambda call: call.data[-1] == 'h')
@logger.catch()
def callback_button(call: CallbackQuery) -> None:
    ...
    if call.message:
        if call.data:
            ...
            bot.set_state(call.from_user.id, SearchToHighPrice.data_in, call.message.chat.id)


@bot.message_handler(state=SearchToHighPrice.data_in)
@logger.catch()
def cal_in(message: Message):
    ...
    if message.text.isdigit():
        ...
        calendar, step = DetailedTelegramCalendar(calendar_id=3, min_date=date.today()).build()
...
def set_date_in(call: CallbackQuery):
    ...
    * edit_message_text - заменяет календарь на выбранную дату.
    """
    result, key, step = DetailedTelegramCalendar(calendar_id=3, min_date=date.today()).process(call.data)
    if not result and key:
        ...
        bot.set_state(call.from_user.id, SearchToHighPrice.data_out, call.message.chat.id)


@bot.message_handler(state=SearchToHighPrice.data_out)
@logger.catch()
def cal_out(message: Message):
    ...
    if message.text.lower() == 'да':
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data_bot:
            calendar, step = DetailedTelegramCalendar(calendar_id=4, min_date=data_bot['data_in']).build()
...
def set_date_out(call: CallbackQuery):
    ...
    * edit_message_text - заменяет календарь на выбранную дату.
    ...
    if not result and key:
        ...
        bot.set_state(call.from_user.id, SearchToHighPrice.inter_state, call.message.chat.id)


@bot.message_handler(state=SearchToHighPrice.inter_state)
@logger.catch()
def question_data(message: Message) -> None:
    ...
    """
    if message.text.lower() == 'да':
        bot.set_state(message.from_user.id, SearchToHighPrice.question_bot, message.chat.id)
...
@bot.message_handler(state=SearchToHighPrice.question_bot)
@logger.catch()
def question_photo(message: Message) -> None:
    ...
    """
    if message.text.lower() == 'да':
        bot.set_state(message.from_user.id, SearchToHighPrice.photo, message.chat.id)
        bot.send_message(message.from_user.id, 'Сколько фотографий одного отеля вам нужно (не больше 25)?')
    elif message.text.lower() == 'нет':
        bot.set_state(message.from_user.id, SearchToHighPrice.answer, message.chat.id)
        bot.send_message(message.from_user.id, 'Верно (да или нет)?', reply_markup=yes_no())
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data_bot:
...
@bot.message_handler(state=SearchToHighPrice.photo)
@logger.catch()
...
@bot.message_handler(state=SearchToHighPrice.answer)
@logger.catch()
    ...
    if message.text.lower() == 'да':
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data_bot:
            ...
            hotels = surf_internet.search_hotel({"destinationId": data_bot['id_group'],
                                                 "sortOrder": "PRICE_HIGHEST_FIRST"}, data_bot['quantity_hotel'], True)
            for hotel in hotels:
                text = print_text.com(hotel, delta_date.days)
                bot.send_message(message.chat.id, text, disable_web_page_preview=True)
                date_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                database.add_command(message.from_user.id, date_time, 'high_price', data_bot['city'], hotel["name"],
